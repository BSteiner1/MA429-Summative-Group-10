...
for(folder in folders){
  ...
  for(image_name in image_names[0:10]){       ## NOTE SHORTENED TO 10 FOR EASE RIGHT NOW
    ...
    img_array <- array(img, dim = c(256, 256, 3))
    ...
    tuple <- list(image = img_array, label = label)
...
```{r}
# Overall, we create a TRAIN:VAL:TEST split of 60:20:20
# First create a train/other split
train_index <- createDataPartition(seq_along(vec_data), p = 0.6, list = FALSE)
train_data <- vec_data[train_index]
other_data <- vec_data[-train_index]

# Use "other" to create a validation/test split
val_index <- createDataPartition(seq_along(vec_data), p = 0.5, list = FALSE)
val_data <- vec_data[val_index]
test_data <- vec_data[-val_index]
```
